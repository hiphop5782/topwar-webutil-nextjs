...
export default function Base() {
    ...
    const getBadgeStyle = useCallback((baseTypes) => {
        if (baseTypes.active) {
            return `badge border border-${baseTypes.color} bg-${baseTypes.color} me-1 pointer`;
        }
        else {
            return `badge border border-${baseTypes.color} text-${baseTypes.color} me-1 pointer`
        }
    }, []);
                ...
                {filterList.map(b => (
                    <div key={b.no} style={{width:250}}>
                        <div className="card mb-3">
                            ...
                            <img src={`/images/base/${b.no}.png`} height={200}/>
                            <div className="card-body">
                                <div className="card-text">
                                    <span className={`badge pointer bg-secondary`}>사용 시 효과</span>
                                    {b.options1.map((opt, idx)=>(
                                        <div key={idx}>
                                        {selectedTypes.length > 0 ? (<>
                                            {selectedTypes.some(st=>opt.name.indexOf(st) >= 0) ? (
                                            <div><small className={styles.highlight}>{opt.name}&nbsp;<span className="text-danger"><b>{opt.value}</b></span></small></div>    
                                            ) : (
